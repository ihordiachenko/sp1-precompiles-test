...
Find your program verification key by going into the `../script` directory and running `RUST_LOG=info cargo run --package hasher-script --bin vkey --release`, which will print an output like:
...
```sh
forge create src/hasher.sol:hasher --rpc-url $RPC_URL --private-key $PRIVATE_KEY --constructor-args $VERIFIER $PROGRAM_VKEY
```
...
```sh
forge create src/hasher.sol:hasher --rpc-url $RPC_URL --private-key $PRIVATE_KEY --constructor-args $VERIFIER $PROGRAM_VKEY --verify --verifier etherscan --etherscan-api-key $ETHERSCAN_API_KEY
```
